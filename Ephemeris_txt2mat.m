...
parfor cnt_file = 1:MAX_COUNT_FILE
    ...
    data = textscan(fid,"%s");
    fclose(fid);
    data = reshape(data{1},[28,4321]);
    datetime_UTC = datetime(data(1,:),"Format","uuuuDDDHHmmss.SSS");
    datevec_UTC = datevec(datetime_UTC);
    pos_ECI = str2double(data(2:4,:))';
    vel_ECI = str2double(data(5:7,:))';
    cov = str2double(data(8:end,:))';
